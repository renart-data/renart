...
export type CodeTypeGlyphKind = "sql" | "python" | "text";

export function CodeTypeGlyph({
  type,
  className,
}: {
  type: CodeTypeGlyphKind;
  className?: string;
}) {
  if (type === "python") {
    return (
  ...
  return (
    <span
      aria-hidden="true"
      data-code-type-glyph={type}
      className={cn(
        "inline-flex size-4 items-center justify-center font-sans font-medium leading-none tracking-tight",
        type === "sql" ? "text-[0.58rem]" : "font-serif text-lg",
        className,
      )}
    >
      {type === "sql" ? "SQL" : "T"}
