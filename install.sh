...
Usage: $this [-b bindir] [-d] [-q] [-n] [-D] [tag]
  ...
  -D installs the DuckDB build when available for the current platform (default)
   [tag] is a tag from
   https://github.com/renart-data/renart/releases
...
Environment variables:
  ...
  RENART_INSTALL_DUCKDB=0    Install the smaller build without DuckDB
...
parse_args() {
  ...
  DRY_RUN=0
  RENART_INSTALL_DUCKDB="${RENART_INSTALL_DUCKDB:-1}"
  while getopts "b:dDqh?xn" arg; do
    case "$arg" in
    b) BINDIR="$OPTARG" ;;
    d) log_set_priority 10 ;;
    D) RENART_INSTALL_DUCKDB=1 ;;
...
resolve_asset_filename() {
  ...
  if [ "${RENART_INSTALL_DUCKDB}" = '1' ] && true
  then
    ASSET_FILENAME="${NAME}_${OS}_${ARCH}_duckdb${EXT}"
  fi
...
asset_is_available() {
  source_url=$1
  if is_command curl; then
    curl -fsLI "$source_url" >/dev/null 2>&1
    return $?
  elif is_command wget; then
    wget -q --spider "$source_url"
    return $?
  fi
  log_crit "asset_is_available unable to find wget or curl"
  return 1
}
...
execute() {
  ...
  if [ "${RENART_INSTALL_DUCKDB}" = "1" ] && ! asset_is_available "${ASSET_URL}"; then
    log_crit "DuckDB build is not available for ${OS}/${ARCH} on ${TAG}. Set RENART_INSTALL_DUCKDB=0 to install the smaller build."
    return 1
  fi
